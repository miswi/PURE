...
from entity.utils import conver_dataset_to_samples,batchify,NpEncoder
...
def evaluate(model,batches,tot_gold):
    ...
    for i in range(len(batches)):
        ...
        for sample,preds in zip(batches[i], pred_ner):
            for gold,pred in zip(sample['spans_labels'],preds):
                l_tot += 1
                if pred == gold:
                    l_cor += 1
                if pred != 0 and gold != 0 and pred == gold:  # 非padding预测正确的
                    cor += 1
                if pred != 0:
                    tot_pred += 1
    ...
    p = cor / tot_pred if cor > 0 else 0.0  # 在预测总量中，预测对的有多大比例
    r = cor / tot_gold if cor > 0 else 0.0  # 在标签总量中，预测对的有多大比例
    f1 = 2 * (p * r) / (p + r) if cor > 0 else 0.0
    logger.info('Precision: %.5f, Recall: %.5f, F1: %.5f'%(p, r, f1))
...
def output_ner_predictions(model,batches,dataset,output_file):
    ...
    logger.info('Total pred entites: %d'%tot_pred_ett)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--task',type=str,default='scierc',required=False,choices=['ace04','ace05','scierc'])
    parser.add_argument('--data_dir',type=str,default=r'./processed_data/json',required=False)
    ...
    parser.add_argument('--num_epoch',type=int,default=30,
                        help='number of thr training epochs')
    parser.add_argument('--print_loss_step',type=int,default=100,
                        help='how often logging the loss value during training')
    parser.add_argument('--eval_per_epoch',type=int,default=1,
                        help='how often evaluating the train model on dev set during training')
    parser.add_argument('--bertadam',action='store_true',default=False,
                        help='if bertadam, the set correct_bias = False')

    parser.add_argument('--do_train',action='store_true',default=False,help='whether to run training')
    parser.add_argument('--train_shuffle',action='store_true',default=True,help='whether to train with randomly shuffled data')
    parser.add_argument('--do_eval',action='store_true',default=False,help='whether to run evaluation')
    parser.add_argument('--eval_test',action='store_true',default=False,help='whether to evaluate on test set')
    ...
    parser.add_argument('--use_albert',action='store_true',help='whether to use ALBERT model')
    parser.add_argument('--model',type=str,default=r'E:\Code File\Pre-train-Model\torchVersion\bert-base-uncased',
                        help='the base model name (a huggingface model)')
    parser.add_argument('--bert_model_dir',type=str,default=None)
    parser.add_argument('--seed',type=int,default=0)
    parser.add_argument('--context_window',type=int,required=False,default=100,
    ...
    model = EntityModel(args,num_ner_labels)
    dev_data = Dataset(args.dev_data)
    dev_samples,dev_ner = conver_dataset_to_samples(dev_data,args.max_span_length,ner_label2id,args.context_window)
    dev_batches = batchify(dev_samples,args.eval_batch_size)

    if args.do_train:
        train_data = Dataset(args.train_data)
        train_samples,train_ner = conver_dataset_to_samples(train_data,args.max_span_length,ner_label2id,args.context_window)
        ...
        optimizer_grouped_parameters = [
            ...
            {'params': [param for name,param in param_optimizer if 'bert' not in name],'lr': args.task_learning_rate},
        ]
        optimizer = AdamW(optimizer_grouped_parameters,lr=args.learning_rate,correct_bias=not(args.bertadam))  # 使用BertAdam
        ...
        for _ in tqdm(range(args.num_epoch)):
            ...
            for i in tqdm(range(len(train_batches))):
                ...
                tr_loss += loss.item()
                tr_exmaples += len(train_batches[i])
                global_step += 1
                ...
                if global_step % args.print_loss_step == 0:
                    logger.info('Epoch = %d, iter = %d, loss= %.5f' % (_, i, tr_loss/tr_exmaples))
                    tr_loss = 0
                    tr_exmaples = 0

            logger.info('Epoch: %d', (_))
            f1 = evaluate(model,dev_batches,dev_ner)
            if f1 > best_result:
                best_result = f1
                logger.info('----Best F1 in valid %.2f in Epoch %d'%(f1*100,_))
    ...
    if args.do_eval:
        ...
        test_samples, test_ner = conver_dataset_to_samples(test_data,args.max_span_length,ner_label2id=ner_label2id,context_window=args.context_window)
