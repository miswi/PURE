...
class BertForRelationApprox(BertPreTrainedModel):
    ...
    def forward(self, input_ids,token_type_ids=None, attention_mask=None, labels=None, sub_obj_ids=None, sub_obj_masks=None, input_position=None):
        ...
        sub_embeddings = batched_index_select(sequence_output,sub_ids)  # (batch_size,max_num_pairs,embed_dim)
        ...
        if labels is not None:
            ...
            # 对于损失函数，只回传非padding的token
            active_labels = torch.where(
                active_loss, labels.view(-1), torch.tensor(loss_fcn.ignore_index).type_as(labels)
            )
            loss = loss_fcn(active_logits, active_labels)
            return loss
        else:
            return logits
